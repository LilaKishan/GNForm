...
public partial class AdminPanel_ACC_Income_ACC_IncomeList : System.Web.UI.Page
{
    ...
    #region 25.0 DeleteMany Button Click
    ...
    protected void btnDeleteSelected_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                CheckBox chkSelect = (CheckBox)item.FindControl("chkSelect");
                HiddenField hfId = (HiddenField)item.FindControl("hfID"); // Assuming you have a HiddenField for ID

                if (chkSelect != null && chkSelect.Checked && hfId != null && !string.IsNullOrEmpty(hfId.Value))
                {
            ...
            if (selectedIds.Count > 0)
            {
                ...
                string ledgerIdsList = string.Join(",", selectedIds);

                ACC_IncomeBAL balACC_Income = new ACC_IncomeBAL();
                if (balACC_Income.DeleteMultiple(ledgerIdsList))
                {
                    // Show success message
                    ucMessage.ShowSuccess("Selected records deleted successfully.");
                ...
                else
                {
                    ...
                    ucMessage.ShowError("Error deleting selected records.");
                }
            }
            else
            {
                // Show message if no records were selected
                ucMessage.ShowInfo("No records selected for deletion.");
            }
        }
        catch (Exception ex)
        {
