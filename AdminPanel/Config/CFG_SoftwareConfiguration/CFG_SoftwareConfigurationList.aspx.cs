...
public partial class AdminPanel_CFG_SoftwareConfiguration_CFG_SoftwareConfigurationList : System.Web.UI.Page
{ 
    #region 11.0 Variables
	
    String FormName = "CFG_SoftwareConfigurationList";
    ...
    private void Search(int PageNo)
    {
        ...
        SqlString SaveMessage_NoMessageJustClosetheform = SqlString.Null;
        SqlString SaveMessage_ShowMessageClosetheform = SqlString.Null;
        SqlString SaveMessage_ShowMessageAskforOtherRecord = SqlString.Null;
        SqlString ShortcutKeys_EditOnEnterKeyinListPage = SqlString.Null;
        SqlString ShortcutKeys_DoubleClicK = SqlString.Null;
        SqlInt32 HospitalID = SqlInt32.Null;
        SqlString WeeklyBackupPassword = SqlString.Null;
		...
		if (txtSaveMessage_NoMessageJustClosetheform.Text.Trim() != String.Empty)
			SaveMessage_NoMessageJustClosetheform = txtSaveMessage_NoMessageJustClosetheform.Text.Trim();

		if (txtSaveMessage_ShowMessageClosetheform.Text.Trim() != String.Empty)
			SaveMessage_ShowMessageClosetheform = txtSaveMessage_ShowMessageClosetheform.Text.Trim();

		if (txtSaveMessage_ShowMessageAskforOtherRecord.Text.Trim() != String.Empty)
			SaveMessage_ShowMessageAskforOtherRecord = txtSaveMessage_ShowMessageAskforOtherRecord.Text.Trim();

		if (txtShortcutKeys_EditOnEnterKeyinListPage.Text.Trim() != String.Empty)
			ShortcutKeys_EditOnEnterKeyinListPage = txtShortcutKeys_EditOnEnterKeyinListPage.Text.Trim();

		if (txtShortcutKeys_DoubleClicK.Text.Trim() != String.Empty)
			ShortcutKeys_DoubleClicK = txtShortcutKeys_DoubleClicK.Text.Trim();

		if (ddlHospitalID.SelectedIndex > 0)
			HospitalID = Convert.ToInt32(ddlHospitalID.SelectedValue);

		if (txtWeeklyBackupPassword.Text.Trim() != String.Empty)
			WeeklyBackupPassword = txtWeeklyBackupPassword.Text.Trim();
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                ...
                {                    
                    if (balCFG_SoftwareConfiguration.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
