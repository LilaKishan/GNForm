...
public partial class AdminPanel_Master_MST_Student_MST_StudentList : System.Web.UI.Page
{
    ...
    String FormName = "MST_StudentList";
    ...
    private void Search(int PageNo)
    {
        ...
        SqlString EmailPersnol = SqlString.Null;
        ...
        if (ddlGender.SelectedIndex > 0)
            Gender = ddlGender.SelectedValue.ToString().Trim();

        if (txtEmailInstitute.Text.Trim() != String.Empty)
            EmailInstitute = txtEmailInstitute.Text.Trim();

        if (txtEmailPersnol.Text.Trim() != String.Empty)
            EmailPersnol = txtEmailPersnol.Text.Trim();
        ...
        DataTable dt = balMST_Student.SelectPage(Offset, PageRecordSize, out TotalRecords, StudentName, EnrollmentNo, RollNo, CurrentSem, EmailInstitute, EmailPersnol, ContactNo, Gender);
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                ...
                {
                    if (balMST_Student.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
    ...
    protected void lbtnExport_Click(object sender, EventArgs e)
    {
        ...
        MST_StudentBAL balMST_Student = new MST_StudentBAL();
        DataTable dtMST_Student = balMST_Student.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord, StudentName, EnrollmentNo, RollNo, CurrentSem, EmailInstitute, EmailPersnol, ContactNo, Gender);
        if (dtMST_Student != null && dtMST_Student.Rows.Count > 0)
        {
            Session["ExportTable"] = dtMST_Student;
    ...
    private void ClearControls()
    {
        ...
        txtEmailPersnol.Text = String.Empty;
