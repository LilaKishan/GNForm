...
public partial class AdminPanel_MST_SubTreatment_MST_SubTreatmentList : System.Web.UI.Page
{ 
    #region 11.0 Variables
	
    String FormName = "MST_SubTreatmentList";
    ...
    private void Search(int PageNo)
    {
        ...
        SqlString SubTreatmentName = SqlString.Null;
        SqlInt32 SequenceNo = SqlInt32.Null;
        SqlDecimal Rate = SqlDecimal.Null;
        SqlInt32 HospitalID = SqlInt32.Null;
        SqlString DefaultUnit = SqlString.Null;
		...
		if (txtSubTreatmentName.Text.Trim() != String.Empty)
			SubTreatmentName = txtSubTreatmentName.Text.Trim();

		if (txtSequenceNo.Text.Trim() != String.Empty)
			SequenceNo = Convert.ToInt32(txtSequenceNo.Text.Trim());
        ...
        DataTable dt = balMST_SubTreatment.SelectPage(Offset, PageRecordSize, out TotalRecords, SubTreatmentName, SequenceNo, Rate, HospitalID, DefaultUnit);
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                ...
                {                    
                    if (balMST_SubTreatment.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
	...
	if (ViewState["CurrentPage"] != null)
        ...
        MST_SubTreatmentBAL balMST_SubTreatment = new MST_SubTreatmentBAL();
        DataTable dtMST_SubTreatment = balMST_SubTreatment.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord, SubTreatmentName, SequenceNo, Rate, HospitalID, DefaultUnit);
        if (dtMST_SubTreatment != null && dtMST_SubTreatment.Rows.Count > 0)
		{
            Session["ExportTable"] = dtMST_SubTreatment;
