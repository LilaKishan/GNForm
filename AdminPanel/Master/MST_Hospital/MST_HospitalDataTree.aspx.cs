...
public partial class AdminPanel_Master_MST_Hospital_MST_HospitalDataTree : System.Web.UI.Page
{
    ...
    private void Search(int PageNo)
    {
        ...
        SqlString PrintLine2 = SqlString.Null;
        SqlString PrintLine3 = SqlString.Null;
        SqlString FooterName = SqlString.Null;
        SqlString ReportHeaderName = SqlString.Null;
        ...
        if (txtPrintLine1.Text.Trim() != String.Empty)
            PrintLine1 = txtPrintLine1.Text.Trim();
        ...
        DataTable dt = balMST_Hospital.SelectPage(Offset, PageRecordSize, out TotalRecords, Hospital, PrintName, PrintLine1, PrintLine2, PrintLine3, FooterName, ReportHeaderName);
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            //foreach (RepeaterItem rpcount in rptHospitals.Items)
            //{
    ...
    protected void rptHospitals_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "LoadFinYears")
        {
            ...
            SqlDateTime OneDateOfMonth = SqlDateTime.Null;
            ...
            if (panelFinYears != null)
            {
                if (panelFinYears.Visible)
                {
                    ...
                    btnshowFinYears.BackColor = System.Drawing.Color.Green; // Set background to green
                }
                else
                {
                    HospitalID = Convert.ToInt32(e.CommandArgument);
                    ...
                    MST_HospitalBAL balMST_Hospital = new MST_HospitalBAL();
                    DataTable dtFinYears = balMST_Hospital.SelectDataTreePage(HospitalID, FinYearID, OneDateOfMonth);

                    // Bind the Financial Years data to the nested repeater
                    btnshowFinYears.Text = "-";
                    btnshowFinYears.BackColor = System.Drawing.Color.LightGreen; // Set background to green
    ...
    protected void rptFinYears_ItemCommand(object sender, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "LoadMonths")
        {
            ...
            Panel panelMonths = (Panel)e.Item.FindControl("pnlMonths");

            if (panelMonths != null)
            {
                if (panelMonths.Visible)
                {
                ...
                else
                {
                    ...
                    HiddenField chdnHospitalID = (HiddenField)e.Item.FindControl("hdnHospitalID");


                    if (hdnHospitalID.Value != string.Empty)
                    {
                        HospitalID = Convert.ToInt32(hdnHospitalID.Value);
                        chdnHospitalID.Value = hdnHospitalID.Value;
                    }
                    if (e.CommandArgument.ToString().Trim() != string.Empty)
                        FinYearID = Convert.ToInt32(e.CommandArgument);

                    // Find the nested repeater for Income
                    Repeater rptMonths = (Repeater)e.Item.FindControl("rptMonths");

                    MST_HospitalBAL balMST_Hospital = new MST_HospitalBAL();
                    DataTable dtMonths = balMST_Hospital.SelectDataTreePage(HospitalID, FinYearID, OneDateOfMonth);


                    // Bind the Month data to the nested repeater
                    rptMonths.DataSource = dtMonths;
                    rptMonths.DataBind();
    ...
    protected void rptMonths_ItemCommand(object sender, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "LoadIncomes")
        {
            ...
            if (panelIncomes != null)
            {
                ...
                else
                {
                    ...
                    RepeaterItem parentItem = (RepeaterItem)((Repeater)sender).NamingContainer;
                    HiddenField hdnHospitalID = (HiddenField)parentItem.FindControl("hdnHospitalID");
                    HiddenField hdnFinYearID = (HiddenField)parentItem.FindControl("hdnFinYearID");

                    HospitalID = Convert.ToInt32(hdnHospitalID.Value);
                    FinYearID = Convert.ToInt32(hdnFinYearID.Value);
                    OneDateOfMonth = Convert.ToDateTime(e.CommandArgument);

                    // Find the nested repeater for Income
                    Repeater rptIncomes = (Repeater)e.Item.FindControl("rptIncomes");

                    // Fetch Incomes for the selected HospitalID and FinYearID
                    MST_HospitalBAL aCC_IncomeListBAL = new MST_HospitalBAL();
                    DataTable dtIncomes = aCC_IncomeListBAL.SelectDataTreePage(HospitalID, FinYearID, OneDateOfMonth);
    ...
    protected void lbtnExport_Click(object sender, EventArgs e)
    {
        ...
        MST_HospitalBAL balMST_Hospital = new MST_HospitalBAL();
        DataTable dtMST_Hospital = balMST_Hospital.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord, Hospital, PrintName, PrintLine1, PrintLine2, PrintLine3, FooterName, ReportHeaderName);
        if (dtMST_Hospital != null && dtMST_Hospital.Rows.Count > 0)
        {
            Session["ExportTable"] = dtMST_Hospital;
    ...
    protected void btnCollapseToggle_Click(object sender, EventArgs e)
    {
    }
