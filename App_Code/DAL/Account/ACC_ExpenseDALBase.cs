...
namespace GNForm3C.DAL
{
	public abstract class ACC_ExpenseDALBase : DataBaseConfig
	{
				...
				using (IDataReader dr = DBH.ExecuteReader(sqlDB, dbCMD))
				{
					while (dr.Read())
					{
                        ...
                        if (!dr["TagName"].Equals(System.DBNull.Value))
                            entACC_Expense.TagName = Convert.ToString(dr["TagName"]);
        ...
        public DataTable SelectPage(SqlInt32 PageOffset, SqlInt32 PageSize, out Int32 TotalRecords, SqlInt32 ExpenseTypeID, SqlString TagName, SqlDecimal Amount, SqlDateTime ExpenseDate, SqlInt32 HospitalID, SqlInt32 FinYearID)
		{
			TotalRecords = 0;
			try
			{
        ...
        public DataTable SelectComboBoxByExpenseTypeWithHospital()
        {
            try
            {
                SqlDatabase sqlDB = new SqlDatabase(myConnectionString);
                DbCommand dbCMD = sqlDB.GetStoredProcCommand("PR_ACC_ExpenseTypeWithHospital_SelectComboBox");

                DataTable dtACC_Expense = new DataTable("PR_ACC_ExpenseTypeWithHospital_SelectComboBox");
